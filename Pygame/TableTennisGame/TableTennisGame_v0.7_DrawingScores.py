## Version 0.7 of a video game version of "table tennis"
## (https://en.wikipedia.org/wiki/Table_tennis).
## This version builds upon the previous version by drawing scores on screen,
## rather than printing them as text to the console.
...
while True:
    ...
    if pressed_keys[RIGHT_PLAYER_UP_KEY]:
        right_player_paddle.MoveUp(time_elapsed_since_last_frame_in_seconds)
    if pressed_keys[RIGHT_PLAYER_DOWN_KEY]:
        right_player_paddle.MoveDown(time_elapsed_since_last_frame_in_seconds)
    ...
    right_paddle_outside_of_screen_top_boundary = (right_player_paddle.BoundingScreenRectangle.top < SCREEN_BOUNDARIES_IN_PIXELS.top)
    ...
    right_paddle_outside_of_screen_bottom_boundary = (right_player_paddle.BoundingScreenRectangle.bottom > SCREEN_BOUNDARIES_IN_PIXELS.bottom)
    ...
    ball_hit_left_paddle = left_player_paddle.BoundingScreenRectangle.collidepoint(
        int(ball.ScreenPosition.X),
        int(ball.ScreenPosition.Y))
    ...
    left_player_score_surface = font.render(str(left_player_score), NO_ANTI_ALIASING, FONT_COLOR)
    ...
    screen.blit(left_player_score_surface, LEFT_SCORE_LEFT_TOP_SCREEN_POSITION)

    right_player_score_surface = font.render(str(right_player_score), NO_ANTI_ALIASING, FONT_COLOR)
    SCORE_PADDING_IN_PIXELS_FROM_TOP = 8
    RIGHT_SCORE_LEFT_TOP_SCREEN_POSITION = (
        SCREEN_BOUNDARIES_IN_PIXELS.right - PLAYER_PADDLE_PIXELS_FROM_SCREEN_SIDE,
        SCREEN_BOUNDARIES_IN_PIXELS.top + SCORE_PADDING_IN_PIXELS_FROM_TOP)
    screen.blit(right_player_score_surface, RIGHT_SCORE_LEFT_TOP_SCREEN_POSITION)
