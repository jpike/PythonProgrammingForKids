...
class Ball(object):
    ...
    def RandomVelocity():
        MAX_VELOCITY_COMPONENT_IN_PIXELS = 64
    ...
    ## @param[in]   screen_position - The Vector2 position of the ball's
    ...
    def Move(self, time_in_seconds_to_move):
        ...
        screen_vector_to_move = Vector2.Scale(
            time_in_seconds_to_move,
            self.VelocityInPixelsPerSecond)
        self.ScreenPosition = Vector2.Add(
            self.ScreenPosition,
...
class PlayerPaddle(object):
    ...
    def MoveUp(self, time_in_seconds_to_move):
        ...
        vertical_move_distance_in_pixels = (
            PlayerPaddle.MOVE_SPEED_IN_PIXELS_PER_SECOND
            *
            time_in_seconds_to_move)
        self.BoundingScreenRectangle = self.BoundingScreenRectangle.move(
            0,
            -vertical_move_distance_in_pixels)
...
INITIAL_BALL_SCREEN_POSITION = Vector2(
    SCREEN_BOUNDARIES_IN_PIXELS.centerx,
...
INITIAL_RIGHT_PLAYER_PADDLE_SCREEN_POSITION = Vector2(
    SCREEN_BOUNDARIES_IN_PIXELS.right - PLAYER_PADDLE_PIXELS_FROM_SCREEN_SIDE,
    SCREEN_BOUNDARIES_IN_PIXELS.centery)
right_player_paddle = PlayerPaddle(INITIAL_RIGHT_PLAYER_PADDLE_SCREEN_POSITION)
...
while True:
    # PROCESS ANY EVENTS SENT TO THE GAME PROGRAM.
    ...
    RIGHT_PLAYER_UP_KEY = pygame.K_UP
    RIGHT_PLAYER_DOWN_KEY = pygame.K_DOWN
    pressed_keys = pygame.key.get_pressed()
    if pressed_keys[LEFT_PLAYER_UP_KEY]:
        left_player_paddle.MoveUp(time_since_last_update_in_seconds)
    if pressed_keys[LEFT_PLAYER_DOWN_KEY]:
        left_player_paddle.MoveDown(time_since_last_update_in_seconds)
    if pressed_keys[RIGHT_PLAYER_UP_KEY]:
        right_player_paddle.MoveUp(time_since_last_update_in_seconds)
    if pressed_keys[RIGHT_PLAYER_DOWN_KEY]:
        right_player_paddle.MoveDown(time_since_last_update_in_seconds)

    # KEEP THE PADDLES WITHIN THE SCREEN.
    left_paddle_outside_of_screen_top_boundary = (
    ...
    if left_paddle_outside_of_screen_top_boundary:
        left_player_paddle.BoundingScreenRectangle.top = SCREEN_BOUNDARIES_IN_PIXELS.top
    left_paddle_outside_of_screen_bottom_boundary = (
    ...
    right_paddle_outside_of_screen_top_boundary = (
        right_player_paddle.BoundingScreenRectangle.top < SCREEN_BOUNDARIES_IN_PIXELS.top)
    if right_paddle_outside_of_screen_top_boundary:
        right_player_paddle.BoundingScreenRectangle.top = SCREEN_BOUNDARIES_IN_PIXELS.top
    right_paddle_outside_of_screen_bottom_boundary = (
        right_player_paddle.BoundingScreenRectangle.bottom > SCREEN_BOUNDARIES_IN_PIXELS.bottom)
    if right_paddle_outside_of_screen_bottom_boundary:
        right_player_paddle.BoundingScreenRectangle.bottom = SCREEN_BOUNDARIES_IN_PIXELS.bottom

    # MOVE THE BALL.
    ball.Move(time_since_last_update_in_seconds)

    # CHECK FOR SCREEN COLLISIONS WITH THE BALL.
    ball_outside_of_screen_left_boundary = (
        ball.ScreenPosition.X < SCREEN_BOUNDARIES_IN_PIXELS.left)
    ball_outside_of_screen_right_boundary = (
        ball.ScreenPosition.X > SCREEN_BOUNDARIES_IN_PIXELS.right)
    ball_outside_of_screen_top_boundary = (
        ball.ScreenPosition.Y < SCREEN_BOUNDARIES_IN_PIXELS.top)
    ball_outside_of_screen_bottom_boundary = (
        ball.ScreenPosition.Y > SCREEN_BOUNDARIES_IN_PIXELS.bottom)
    if ball_outside_of_screen_left_boundary:
        # RESET THE BALL.
        ball = Ball(INITIAL_BALL_SCREEN_POSITION)

        # GIVE A POINT TO THE RIGHT PLAYER.
        right_player_score = right_player_score + 1
    elif ball_outside_of_screen_right_boundary:
        # RESET THE BALL.
        ball = Ball(INITIAL_BALL_SCREEN_POSITION)

        # GIVE A POINT TO THE LEFT PLAYER.
    ...
    # CHECK FOR PADDLE COLLISIONS WITH THE BALL.
    ball_hit_left_paddle = left_player_paddle.BoundingScreenRectangle.collidepoint(
        ball.ScreenPosition.X,
        ball.ScreenPosition.Y)
    if ball_hit_left_paddle:
        # REFLECT THE BALL ACROSS THE X AXIS.
        ball.VelocityInPixelsPerSecond.X *= -1
    
    ball_hit_right_paddle = right_player_paddle.BoundingScreenRectangle.collidepoint(
        ball.ScreenPosition.X,
        ball.ScreenPosition.Y)
    if ball_hit_right_paddle:
        # REFLECT THE BALL ACROSS THE X AXIS.
        ball.VelocityInPixelsPerSecond.X *= -1
    ...
    pygame.draw.rect(
        screen,
        right_player_paddle.Color,
        right_player_paddle.BoundingScreenRectangle)
    ...
    left_player_score_surface = font.render(
        str(left_player_score),
        NO_ANTI_ALIASING,
        FONT_COLOR)
    SCORE_PADDING_IN_PIXELS_FROM_TOP = 8
    LEFT_SCORE_LEFT_TOP_SCREEN_POSITION = (
        SCREEN_BOUNDARIES_IN_PIXELS.left + PLAYER_PADDLE_PIXELS_FROM_SCREEN_SIDE,
        SCREEN_BOUNDARIES_IN_PIXELS.top + SCORE_PADDING_IN_PIXELS_FROM_TOP)
    screen.blit(
        left_player_score_surface,
        LEFT_SCORE_LEFT_TOP_SCREEN_POSITION)

    right_player_score_surface = font.render(
        str(right_player_score),
        NO_ANTI_ALIASING,
        FONT_COLOR)
    SCORE_PADDING_IN_PIXELS_FROM_TOP = 8
    RIGHT_SCORE_LEFT_TOP_SCREEN_POSITION = (
        SCREEN_BOUNDARIES_IN_PIXELS.right - PLAYER_PADDLE_PIXELS_FROM_SCREEN_SIDE,
        SCREEN_BOUNDARIES_IN_PIXELS.top + SCORE_PADDING_IN_PIXELS_FROM_TOP)
    screen.blit(
        right_player_score_surface,
        RIGHT_SCORE_LEFT_TOP_SCREEN_POSITION)
